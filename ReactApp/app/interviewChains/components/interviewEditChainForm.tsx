...
interface InterviewEditChainFormProps {
  initialData: InterviewChain | null;
  interviewChainId: number;
  onSubmit: (data: InterviewChain) => void;
  loading?: boolean;
}

export default function InterviewEditChainForm({
  initialData,
  interviewChainId,
  onSubmit,
  loading,
}: InterviewEditChainFormProps) {
  const { isAuthenticated, isInitialized, roles, login } = useAuth();
  const router = useRouter();
  const [formData, setFormData] = useState<Partial<InterviewChain>>(
    initialData || {
      InterviewChainID: interviewChainId,
      ClientID: undefined,
      EndClientName: "",
      InterviewType: "",
      InterviewDate: "",
      ...
      Comments: "", // Added default value for Comments
    }
  );
  ...
  useEffect(() => {
    ...
    checkAccess();
  }, [isAuthenticated, isInitialized, router, login, canEdit]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!canEdit) {
      setError(
        "Unauthorized: You do not have permission to edit interview chains."
      );
      return;
    }
    ...
    setIsLoading(true);
    setError(null);
    onSubmit(formData as InterviewChain); // Assert as InterviewChain since validation passed
    setIsLoading(false);
  };
                  ...
                  <Grid container spacing={0.5}>
                    ...
                    <Grid item xs={12} sx={{ mb: 1 }}>
                      ...
                      >
                        ...
                        >
                          {isLoading ? "Updating..." : "Update Interview Chain"}
                        </Button>
                      </Box>
                    </Grid>
                  </Grid>
