...
export default function InterviewEditForm({
  initialData,
  interviewId,
  onSubmit,
  loading,
  ...
  const canEdit = isAuthenticated && permissions.interviews[userRole]?.editInterview === true;
  ...
  useEffect(() => {
    ...
    if (!canEdit) {
      setError("Unauthorized: You do not have permission to edit interviews.");
      return;
    }
    if (initialData) {
      setFormData(initialData);
      if (initialData.RecruiterID) {
    ...
    fetchRecruiters();
  }, [isAuthenticated, router, userRole, initialData, canEdit]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setError(null);
    try {
      const submitData: InterviewDetail = {
        ...formData,
        InterviewEntryDate: formData.InterviewEntryDate
          ? new Date(formData.InterviewEntryDate).toISOString().split("T")[0]
          : null,
      ...
      await updateInterview(interviewId, submitData);
                ...
                <Grid container spacing={0.5} alignItems="center">
                  <Grid item xs={12} sm={4} sx={{ mb: 1 }}>
                    <TextField
                      ...
                      type="date"
                      value={formData.InterviewDate ? new Date(formData.InterviewDate).toISOString().split("T")[0] : ""}
        ...
        {permissions.interviews[userRole]?.basicInfo?.view && (
          <Grid item xs={12} sx={{ mb: 1 }}>
            <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 2, mt: 3 }}>
              ...
              <Button type="submit" variant="contained" color="primary" disabled={isLoading || !canEdit}>
